...
export function useEventManagement({
  ...
  const checkDuplicateEventName = (
    name: string,
    excludeEventId?: string
  ): void => {
    const existingEvent = events.find(
      (event) =>
        (!excludeEventId || event.id !== excludeEventId) && event.name === name
    );

    if (existingEvent) {
      throw new Error(
        `An event named "${name}" already exists in this occasion`
      );
    }
  };

  const uploadEventInvite = async (
    file: File,
    eventId: string,
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `${occasionAlias}/events/${eventId}/event-invite.${fileExtension}`
      );
  ...
  const handleAddEvent = async (
    ...
    try {
      ...
      if (imageFile) {
        const downloadUrl = await uploadEventInvite(
          imageFile,
          docRef.id,
          eventData.occasionAlias
        );
  ...
  const handleUpdateEvent = async (
    ...
    if (!eventData.id || !eventData.name || !eventData.date || !eventData.time)
      return;
    ...
    try {
      ...
      if (imageFile) {
        ...
        finalImageUrl = await uploadEventInvite(
          imageFile,
          eventData.id,
          eventData.occasionAlias
        );
      ...
      try {
        if (eventToDelete.occasionAlias) {
          const eventStorageRef = ref(
            storage,
            `${eventToDelete.occasionAlias}/events/${eventToDelete.id}`
          );
          const storageList = await listAll(eventStorageRef);

          const deletePromises = storageList.items.map((item) =>
            deleteObject(item)
          );
